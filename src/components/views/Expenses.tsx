...
const EXPENSE_CATEGORIES = ['Rent', 'Transport', 'Electricity', 'Salary', 'Inventory', 'Maintenance', 'Other'];
...
const Expenses: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await addExpense({ category, amount, note, date: new Date() });
      closeModal();
    } catch (err) {
  ...
  return (
    <div className="pb-32 animate-fade-in">
                ...
                {items.map(expense => (
                  <Card key={expense.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-red-50 rounded-md text-red-500"><Tag size={16} /></div>
                      <div>
                    ...
                    <div className="flex items-center gap-3">
                       ...
                       <button onClick={() => handleDelete(expense.id!)} className="p-2 text-red-400 hover:bg-red-50 rounded-full active:scale-90 transition-all">
                         <Trash2 size={16} />
                       </button>
                    </div>
                  </Card>
      ...
      <BottomSheet isOpen={isModalOpen} onClose={closeModal} title={t('expenses.addExpense')}
        footer={<Button onClick={handleSave} isLoading={isProcessing} icon={<Plus size={20} />}>{t('common.save')}</Button>}>
        <div className="space-y-6">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {EXPENSE_CATEGORIES.map(cat => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${category === cat ? 'bg-orange text-white shadow-float' : 'bg-alabaster text-prussian hover:bg-gray-200'}`}>
                  {cat}
                </button>
              ))}
            </div>
          ...
          <AmountInput value={amount} onChange={setAmount} label={t('expenses.amount')} />
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full p-3 bg-white border border-gray-border rounded-md focus:ring-2 focus:ring-orange focus:outline-none min-h-[80px] text-body"
            />
          </div>
        </div>
      </BottomSheet>
