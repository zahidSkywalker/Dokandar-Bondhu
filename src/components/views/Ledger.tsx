...
import { Plus, User, Phone, MapPin, Wallet, Edit3, Trash2, List, CheckCircle, RefreshCw, AlertTriangle, CreditCard, ArrowDownRight, TrendingUp } from 'lucide-react';
...
const Ledger: React.FC = () => {
  ...
  const openStatement = async (customer: any) => {
    ...
    const payments = await db.payments.where('customerId').equals(customer.id).reverse().toArray();
    ...
    const transactions = [
      ...sales.map(s => ({
        ...
        amount: s.total,
        balance: 0 // Placeholder for running balance
      })),
      ...payments.map(p => ({
        id: p.id,
        date: new Date(p.date).getTime(),
        type: 'Payment',
        desc: p.note ? `Payment: ${p.note}` : 'Payment',
    ...
    <div className={`pb-32 max-w-3xl mx-auto min-h-screen ${theme === 'dark' ? 'bg-gray-900 text-gray-100' : 'bg-[#F9FAFB] text-gray-800'}`}>
      ...
      <div className="flex justify-between items-center mb-8 mt-4">
        <div>
          ...
          <p className={`text-base mt-2 font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>Manage debts, credits, and repayments.</p>
        </div>
        <button 
      ...
      {/* Customer List - Increased Card Size and Spacing */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {!customers ? <div className="p-8 text-center">Loading...</div> : customers.length === 0 ? (
          <div className="p-16 text-center border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-3xl">
             ...
             <button onClick={() => setIsAddModalOpen(true)} className="mt-4 bg-gray-100 dark:bg-gray-800 px-6 py-3 rounded-xl font-bold text-gray-700 dark:text-gray-300">
               Add First Customer
             </button>
          </div>
        ) : (
          customers.map((customer) => (
            <div key={customer.id} className={`p-6 rounded-3xl border transition-all hover:shadow-lg ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              
              {/* Header Section */}
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-4">
                  <div className={`p-4 rounded-2xl ${theme === 'dark' ? 'bg-earth-800/20 text-earth-500' : 'bg-earth-50 text-earth-600'}`}>
                    <User size={28} />
                  </div>
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {customer.phone && (
                        <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-700 text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                ...
                {/* Status Badge */}
                <div className={`px-4 py-2 rounded-xl font-bold text-sm ${customer.debt > 0 ? 'bg-red-100 text-red-700 dark:bg-red-900/20 dark:text-red-400' : 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400'}`}>
                  {customer.debt > 0 ? 'Due' : 'Cleared'}
                </div>
              </div>
              
              {/* Balance Section */}
              <div className={`mt-4 pt-4 border-t ${theme === 'dark' ? 'border-gray-700' : 'border-gray-100'}`}>
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button 
                   ...
                   className="py-3 px-4 rounded-2xl text-sm font-bold bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border border-blue-100 dark:border-blue-900 hover:bg-blue-200 transition-colors flex items-center justify-center gap-2 shadow-sm"
                >
      ...
      <Modal isOpen={isManageModalOpen} onClose={() => setIsManageModalOpen(false)} title="Manage Debtor">
        <form onSubmit={handleUpdateDebtor} className="space-y-4">
           <div className="mb-4">
             ...
             <input disabled value={selectedCustomer?.name} className="w-full p-4 bg-gray-100 dark:bg-gray-700 rounded-xl text-gray-500 dark:text-gray-400 text-lg font-bold" />
           </div>
      ...
      <Modal isOpen={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} title="Record Payment">
        <form onSubmit={handlePayment} className="space-y-4">
          <div className={`p-6 rounded-2xl mb-6 flex justify-between items-end ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'}`}>
             <div>
                <p className="text-xs uppercase font-bold text-gray-500 dark:text-gray-400 mb-2">{t('ledger.debt')}</p>
                <p className={`text-4xl font-bold ${selectedCustomer?.debt > 0 ? 'text-red-500 dark:text-red-400' : 'text-green-500 dark:text-green-400'}`}>
                  {formatCurrency(selectedCustomer?.debt || 0, lang)}
                </p>
             </div>
             {selectedCustomer?.debt > 0 && (
                <button onClick={() => { setIsPaymentModalOpen(false); openStatement(selectedCustomer); }} className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-700 px-3 py-2 rounded-lg font-bold">
                   See History
                </button>
             )}
          </div>
      ...
      <Modal isOpen={isStatementModalOpen} onClose={() => setIsStatementModalOpen(false)} title={`Statement: ${selectedCustomer?.name}`}>
        <div className="space-y-4">
           {/* Header Summary */}
           <div className={`p-6 rounded-2xl mb-6 flex justify-between items-end border ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-100'}`}>
             <div>
                <p className="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Total Balance</p>
             ...
             {selectedCustomer?.debt > 0 && (
                <button onClick={() => { setIsStatementModalOpen(false); openPaymentModal(selectedCustomer); }} className="bg-green-100 dark:bg-green-900/30 text-green-700 px-4 py-2 rounded-lg font-bold text-sm shadow-sm hover:bg-green-200">
                   Pay Now
                </button>
             )}
           </div>
                    ...
                    <div key={index} className={`grid grid-cols-4 gap-2 p-3 border-b last:border-0 ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'} items-center`}>
                      ...
                      <div className="flex flex-col items-center gap-1">
                        {tx.type === 'Sale' ? (
                           ...
                           <div className={`p-1.5 rounded-md ${theme === 'dark' ? 'bg-red-900/20' : 'bg-red-50'}`}>
                              <ArrowDownRight size={14} className="text-red-600 dark:text-red-400"/>
                           </div>
                        )}
                        <span className="text-xs font-bold text-gray-700 dark:text-gray-300">{tx.type}</span>
                      </div>
                      ...
                      <div className={`text-right font-bold ${tx.amount >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
