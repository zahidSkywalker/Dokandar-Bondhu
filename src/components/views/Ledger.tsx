...
const Ledger: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        // EXPLICIT CHECK: Check if 'customers' table exists in DB
        if (!db.customers) {
          throw new Error("Database table 'customers' not found. Database might need clearing.");
        }
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Manually refresh to ensure state update
      if (db.customers) {
  ...
  return (
    <div className={`pb-24 max-w-2xl mx-auto ${theme === 'dark' ? 'bg-gray-900 text-gray-100' : 'bg-cream-50 text-earth-900'}`}>
      ...
      {/* Error Display (Crash Protection) */}
      {dbError && (
        <div className="p-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900 rounded-xl flex flex-col items-center justify-center text-center mb-6">
          ...
          <p className="text-red-600 dark:text-red-400 font-bold">Database Error</p>
