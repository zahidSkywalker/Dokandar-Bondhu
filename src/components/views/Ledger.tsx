...
const Ledger: React.FC = () => {
  ...
  // FIX: Ensure query matches table name exactly ('customers')
    ...
    <div className={`pb-24 max-w-2xl mx-auto min-h-screen ${theme === 'dark' ? 'bg-gray-900 text-gray-100' : 'bg-cream-50 text-earth-900'}`}>
      ...
      <div className="space-y-4">
        {!customers ? <p className="text-center p-4">Loading...</p> : customers.length === 0 ? (
          <p className={`text-center text-gray-400 mt-10 ${theme === 'dark' ? 'bg-gray-800 p-10 rounded-2xl' : 'bg-white p-10 rounded-2xl border-cream-200'}`}>No customers yet</p>
        ) : (
          customers.map((customer) => (
            <div key={customer.id} className={`p-5 rounded-2xl border shadow-sm flex justify-between items-center ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-cream-200'}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {customer.phone && <p className={`text-xs flex items-center gap-1 ${theme === 'dark' ? 'text-gray-400' : 'text-earth-500'}`}><Phone size={12}/> {customer.phone}</p>}
                  {customer.address && <p className={`text-xs flex items-center gap-1 ${theme === 'dark' ? 'text-gray-400' : 'text-earth-500'}`}><MapPin size={12}/> {customer.address}</p>}
                </div>
              </div>
