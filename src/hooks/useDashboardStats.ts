...
export const useDashboardStats = () => {
  ...
  // Net Profit = Sales Profit - (General + Inventory Expenses)
  ...
  const chartData = Array.from({ length: 7 }).map((_, i) => {
    ...
    const daySales = allSales?.filter(s => {
      const saleTime = new Date(s.date).getTime(); // Convert to Number to fix build error
  ...
  return {
    ...
    totalDebt: allCustomers?.reduce((sum, c) => sum + c.debt, 0) || 0,
    
    // Monthly Stats (Required by Dashboard)
